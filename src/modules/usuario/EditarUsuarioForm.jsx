  ...
  editarUsuarioSubmit () {
    if (this.validarUpdateForm(ReactDOM.findDOMNode(this.nombreInput).value)) {
      this.props.updateUsuario(
        ...
        this.state.updateForm.organismo.seleccionado, null, null)
    }
  }
    ...
    return (
      <form>
        <Row>
          ...
          <Col lg={4} md={4}>
            <CustomFormField bsSize="small" controlId="email" label="Email" 
              inputComponent={
                <FormControl key="emailInput" defaultValue={this.props.activeUser.email} type="text"
                  readOnly={true} bsSize="small"/>
              }/>
          </Col>
        </Row>
      </form>
...
const mapDispatch = (dispatch) => ({
  updateUsuario: (idUsuario, nombreUsuario, organismo, password, confirmacionPassword) => {
    dispatch(updateUsuario(idUsuario, nombreUsuario, organismo, password, confirmacionPassword))
  }
})
